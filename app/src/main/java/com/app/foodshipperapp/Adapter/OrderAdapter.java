...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.OrderViewHolder> {
    ...
    private void showOrderDetailDialog(Order order) {
        ...
        Button btnAcceptOrder = dialog.findViewById(R.id.btnDoneOrder);
        TextView textOrderIdDetail = dialog.findViewById(R.id.textOrderIdDetail);

        textOrderIdDetail.setText("Order ID: " + order.getOrderId());

        // Lấy thông tin shipper từ SharedPreferences trong Adapter
        SharedPreferences sharedPreferences = context.getSharedPreferences("ShipperAppPrefs", Context.MODE_PRIVATE);
        ...
        if ("received".equalsIgnoreCase(order.getStatus())) {
            ...
            btnAcceptOrder.setOnClickListener(v -> {
                if (shipper != null && shipper.getShipperId() > 0) {
                    // Gán shipperId từ đối tượng Shipper vào order trước khi gửi request
                    order.setShipperId(shipper.getShipperId());  // Lấy shipperId từ thông tin đã lưu

                    // Gọi API để chấp nhận đơn hàng
                    OrderAPI orderAPI = OrderAPI.orderAPI;
                    Call<Order> call = orderAPI.shippingOrder(String.valueOf(order.getOrderId()), order);
                    ...
                    Toast.makeText(context, "Shipper information not found", Toast.LENGTH_SHORT).show();
                }
            });
